...
function applyTimeFilter(chartKey) {
    ...
    // ── SWIPE NAVIGATIE ──────────────────────────────────────────────────────
    const TABS = ['test-portfolio-page', 'data-page', 'aandelen-page', 'heatmap-page', 'analyse-page', 'earnings-page'];
...
function renderWeeklyReturns() {
    ...
    const DEFAULT_STOCKS = [
        ...
        {ticker:'OUST',    name:'Ouster',           type:'stock', currency:'USD', broker:'Degiro', gak:25.61,  aantal:48},
        ...
        // ── SAXO — aandelen ─────────────────────────────────────────────────
        {ticker:'QBTS',    name:'D-Wave',           type:'stock', currency:'USD', broker:'Saxo',   gak:18.2,  aantal:80},
...
let _apCash = JSON.parse(localStorage.getItem('ap_cash_v3') || 'null') || { Bolero: 21, Degiro: 2048, Saxo: 2555 };
...
async function fetchStockEarnings(ticker) {
    const FINNHUB_KEY = 'd4g9371r01qm5b34gb90d4g9371r01qm5b34gb9g';
    const today = new Date().toISOString().slice(0, 10);
    const cacheKey = `earn_v1_${ticker}`;
    try {
    ...
    try {
        const [epsRes, calRes] = await Promise.all([
            fetch(`https://finnhub.io/api/v1/stock/earnings?symbol=${encodeURIComponent(ticker)}&limit=4&token=${FINNHUB_KEY}`),
            fetch(`https://finnhub.io/api/v1/calendar/earnings?from=${today}&to=${in90d}&symbol=${encodeURIComponent(ticker)}&token=${FINNHUB_KEY}`)
        ]);
        const eps = epsRes.ok ? await epsRes.json() : [];
        const cal = calRes.ok ? await calRes.json() : {};
        const quarters = Array.isArray(eps) ? eps.slice(0, 4) : [];
        const nextDate = cal?.earningsCalendar?.[0]?.date ?? null;
        const data = { quarters, nextDate };
        try { localStorage.setItem(cacheKey, JSON.stringify({ date: today, data })); } catch {}
        return data;
    } catch {
        return { quarters: [], nextDate: null };
    }
}

async function initEarningsPage(force = false) {
    const statusEl = document.getElementById('earningsStatus');
    const wrapEl = document.getElementById('earnings-table-wrap');
    if (!wrapEl) return;

    const DEFAULT_STOCKS_EARN = [
        {ticker:'NVDA',name:'NVIDIA Corporation',type:'stock'},{ticker:'PYPL',name:'PayPal Holdings',type:'stock'},
        {ticker:'BMNR',name:'Bitmine Immersion Technologies',type:'stock'},{ticker:'ABCL',name:'AbCellera Biologics',type:'stock'},
        {ticker:'AMZN',name:'Amazon.com',type:'stock'},{ticker:'APLD',name:'Applied Digital Corporation',type:'stock'},
        {ticker:'ACHR',name:'Archer Aviation',type:'stock'},{ticker:'CRDO',name:'Credo Technology Group',type:'stock'},
        {ticker:'IONQ',name:'IonQ',type:'stock'},{ticker:'LMND',name:'Lemonade',type:'stock'},
        {ticker:'MSFT',name:'Microsoft Corporation',type:'stock'},{ticker:'OUST',name:'Ouster',type:'stock'},
        {ticker:'UBER',name:'Uber Technologies',type:'stock'},{ticker:'GOOGL',name:'Alphabet',type:'stock'},
        {ticker:'SOFI',name:'SoFi Technologies',type:'stock'},{ticker:'CRM',name:'Salesforce',type:'stock'},
    ];

    let allHmStocks = JSON.parse(localStorage.getItem('hm_stocks_v2') || 'null') || DEFAULT_STOCKS_EARN;
    allHmStocks = allHmStocks.map(s => ({ type: 'stock', ...s }));
    const stocks = allHmStocks.filter(s => s.type === 'stock');

    if (!stocks.length) {
        wrapEl.innerHTML = '<p style="padding:20px;color:var(--text-muted);">Geen aandelen gevonden. Voeg ze toe via DATA.</p>';
        return;
    }
    ...
    if (force) {
        ...
        stocks.forEach(s => {
            try {
                const cached = JSON.parse(localStorage.getItem(`earn_v1_${s.ticker}`) || 'null');
                if (cached && cached.date === today) localStorage.removeItem(`earn_v1_${s.ticker}`);
            } catch {}
        });
    }

    wrapEl.innerHTML = '<div style="padding:32px;text-align:center;color:var(--text-muted);font-size:0.85rem;">Earnings laden…</div>';
    ...
    for (let i = 0; i < stocks.length; i += batchSize) {
        ...
        const batchData = await Promise.all(batch.map(s => fetchStockEarnings(s.ticker)));
        batch.forEach((s, j) => results.push({ ...s, earnings: batchData[j] }));
        if (i + batchSize < stocks.length) await new Promise(r => setTimeout(r, 150));
    }

    renderEarningsTable(results);
    renderEarningsSummary(results);

    if (statusEl) {
        statusEl.innerHTML = `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--success);margin-right:5px;vertical-align:middle;"></span>Bijgewerkt: ${new Date().toLocaleTimeString('nl-NL', { hour: '2-digit', minute: '2-digit' })}`;
    }
}

function renderEarningsSummary(rows) {
    const el = document.getElementById('earnings-summary');
    if (!el) return;

    const now = new Date();
    const daysUntil = d => d ? Math.ceil((new Date(d + 'T12:00:00') - now) / 86400000) : null;

    const thisWeek = rows.filter(r => { const d = daysUntil(r.earnings?.nextDate); return d !== null && d >= 0 && d <= 7; }).length;

    const withResult = rows.filter(r => r.earnings?.quarters?.[0]?.actual != null && r.earnings?.quarters?.[0]?.estimate != null);
    const beats = withResult.filter(r => {
        const q = r.earnings.quarters[0];
        const p = q.surprisePercent != null ? q.surprisePercent : (Math.abs(q.estimate) > 0.001 ? ((q.actual - q.estimate) / Math.abs(q.estimate)) * 100 : 0);
        return p > 0.5;
    });
    const beatRate = withResult.length ? Math.round((beats.length / withResult.length) * 100) : null;

    const surprises = withResult.map(r => {
        const q = r.earnings.quarters[0];
        return q.surprisePercent != null ? q.surprisePercent : (Math.abs(q.estimate) > 0.001 ? ((q.actual - q.estimate) / Math.abs(q.estimate)) * 100 : 0);
    }).filter(p => Math.abs(p) > 0.5);
    const avgSurprise = surprises.length ? surprises.reduce((a, b) => a + b, 0) / surprises.length : null;

    const stats = [
        { label: 'Gevolgde aandelen', value: rows.length, sub: 'aandelen in lijst' },
        { label: 'Earnings deze week', value: thisWeek, sub: thisWeek === 1 ? 'aandeel rapporteert' : 'aandelen rapporteren' },
        { label: 'Beat rate', value: beatRate !== null ? `${beatRate}%` : '—', sub: `van ${withResult.length} met resultaat` },
        { label: 'Gem. verrassing', value: avgSurprise !== null ? `${avgSurprise > 0 ? '+' : ''}${avgSurprise.toFixed(1)}%` : '—', sub: 'beats & misses' },
    ];

    el.innerHTML = stats.map(s => `
        <div class="earn-stat-chip">
            <div class="earn-stat-label">${s.label}</div>
            <div class="earn-stat-value">${s.value}</div>
            <div class="earn-stat-sub">${s.sub}</div>
        </div>`).join('');
}

function renderEarningsTable(rows) {
    const el = document.getElementById('earnings-table-wrap');
    if (!el) return;
    ...
    const GROUP_LABELS = ['DEZE WEEK', 'VOLGENDE WEEK', 'DEZE MAAND', 'LATER', 'GEEN DATUM', 'VORIGE MAAND', 'AL GEWEEST'];
    const GROUP_COLORS = ['#e74c3c', '#f39c12', '#3498db', 'var(--text-muted)', 'var(--text-muted)', '#8e44ad', 'var(--text-muted)'];

    const sorted = [...rows].sort((a, b) => {
        const ga = getGroup(a.earnings?.nextDate), gb = getGroup(b.earnings?.nextDate);
        if (ga !== gb) return ga - gb;
        const da = a.earnings?.nextDate || 'z', db = b.earnings?.nextDate || 'z';
        // VORIGE MAAND: meest recent bovenaan
        if (ga === 5) return db.localeCompare(da);
        return da.localeCompare(db);
    });

    const groupCounts = {};
    for (const r of sorted) {
        const g = getGroup(r.earnings?.nextDate);
        groupCounts[g] = (groupCounts[g] || 0) + 1;
    }

    const earnBadge = (pct, actual, estimate) => {
        if (actual == null || estimate == null) return '<span style="color:var(--text-muted);font-size:11px;">—</span>';
        const p = pct != null ? pct : (Math.abs(estimate) > 0.001 ? ((actual - estimate) / Math.abs(estimate)) * 100 : 0);
        if (p > 0.5)  return `<span class="earn-badge earn-beat">↑ BEAT +${p.toFixed(1)}%</span>`;
        if (p < -0.5) return `<span class="earn-badge earn-miss">↓ MISS ${p.toFixed(1)}%</span>`;
        return `<span class="earn-badge earn-inline">→ IN LINE</span>`;
    };

    const fmtEps = v => v != null ? `$${v.toFixed(2)}` : '—';

    const fmtDateCell = (d, group) => {
        if (!d) return '<span style="color:var(--text-muted)">—</span>';
        const diff = daysUntil(d);
        const dateObj = new Date(d + 'T12:00:00');
        const label = dateObj.toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' });
        const dayName = group <= 1 ? dateObj.toLocaleDateString('nl-NL', { weekday: 'long' }) : '';
        const isToday = diff === 0;
        let urgency = '';
        if (group === 0) {
            const urgencyText = isToday ? 'VANDAAG' : diff === 1 ? 'MORGEN' : `${diff} DAGEN`;
            urgency = `<span class="earn-urgency${isToday ? ' earn-urgency-today' : ''}" style="background:rgba(231,76,60,.12);color:#e74c3c;">${urgencyText}</span>`;
        } else if (group === 1) {
            urgency = `<span class="earn-urgency" style="background:rgba(243,156,18,.12);color:#f39c12;">${diff} DAGEN</span>`;
        }
        const sub = diff >= 0 ? `over ${diff}d` : `${Math.abs(diff)}d geleden`;
        return `<div style="display:flex;flex-direction:column;align-items:center;gap:3px;">
            ${urgency}
            ${dayName ? `<span style="font-size:9px;color:var(--text-muted);text-transform:capitalize;">${dayName}</span>` : ''}
            <span style="font-size:0.8rem;white-space:nowrap;font-weight:${group <= 1 ? 600 : 400};">${label}</span>
            <span style="font-size:10px;color:var(--text-muted);">${sub}</span>
        </div>`;
    };

    const miniChart = (quarters) => {
        if (!quarters?.length) return '<span style="color:var(--text-muted);font-size:11px;">—</span>';
        const q = [...quarters].slice(0, 4).reverse();
        const W = 96, H = 52, barW = 14, gap = 8;
        const LABEL_H = 11; // pixels reserved for Q-labels at bottom
        const chartH = H - LABEL_H;
        const actuals = q.map(x => x.actual ?? 0);
        const estimates = q.map(x => x.estimate ?? 0);
        const allVals = [...actuals, ...estimates];
        const maxAbs = Math.max(...allVals.map(Math.abs), 0.01);
        // baseY is the zero-line; leave space for negative bars + labels
        const baseY = Math.round(chartH * 0.58);
        const scaleH = (baseY - 3) / maxAbs;

        let bars = '', dots = '', labels = '';
        q.forEach((qtr, i) => {
            const x = i * (barW + gap);
            const act = qtr.actual ?? 0;
            const est = qtr.estimate ?? 0;
            const p = qtr.surprisePercent;
            const isBeat = p > 0.5, isMiss = p < -0.5;
            const col = qtr.actual == null ? '#8e9196' : isBeat ? '#27ae60' : isMiss ? '#e74c3c' : '#8e9196';

            const barH = Math.max(Math.abs(act) * scaleH, 2);
            const barY = act >= 0 ? baseY - barH : baseY;
            const d = new Date((qtr.period || '') + 'T12:00:00');
            const qLabel = qtr.period ? `Q${Math.ceil((d.getMonth()+1)/3)}'${d.getFullYear().toString().slice(2)}` : '';
            bars += `<rect x="${x}" y="${barY.toFixed(1)}" width="${barW}" height="${barH.toFixed(1)}" fill="${col}" rx="2" opacity="0.85"><title>${qLabel}: Act $${act.toFixed(2)} / Est $${est.toFixed(2)}</title></rect>`;

            const dotY = est >= 0 ? baseY - Math.abs(est) * scaleH : baseY + Math.abs(est) * scaleH;
            dots += `<circle cx="${(x + barW/2).toFixed(1)}" cy="${Math.max(1, Math.min(chartH - 1, dotY)).toFixed(1)}" r="2" fill="#7f8c8d" opacity="0.7"><title>Est $${est.toFixed(2)}</title></circle>`;

            if (qLabel) labels += `<text x="${(x + barW/2).toFixed(1)}" y="${H - 1}" text-anchor="middle" font-size="7.5" fill="var(--text-muted)" opacity="0.65">${qLabel}</text>`;
        });

        return `<svg width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" style="display:block;overflow:hidden;">
            <line x1="0" y1="${baseY}" x2="${W}" y2="${baseY}" stroke="var(--border-color)" stroke-width="1"/>
            ${bars}${dots}${labels}
        </svg>`;
    };
    ...
    const quarterLabel = q => {
        if (!q?.period) return '';
        const d = new Date(q.period + 'T12:00:00');
        return `Q${Math.ceil((d.getMonth()+1)/3)} ${d.getFullYear()}`;
    };

    const thS = 'padding:10px 14px;border-bottom:1px solid var(--border-color);font-size:0.7rem;letter-spacing:0.05em;color:var(--text-muted);font-weight:600;white-space:nowrap;text-transform:uppercase;';

    let html = `<table class="earn-table">
        <thead><tr>
            <th style="${thS}text-align:left;">Aandeel</th>
            <th style="${thS}text-align:center;">EPS Historie</th>
            <th style="${thS}text-align:left;">Laatste kwartaal</th>
            <th style="${thS}text-align:left;">Beat / Miss</th>
            <th style="${thS}text-align:center;">Volgende datum</th>
        </tr></thead>
        <tbody>`;

    let lastGroup = -1;
    for (const r of sorted) {
        const quarters = r.earnings?.quarters || [];
        const current = quarters[0] || null;
        const logoUrl = `https://assets.parqet.com/logos/symbol/${r.ticker}?format=png`;
        const group = getGroup(r.earnings?.nextDate);
        const diff = daysUntil(r.earnings?.nextDate);
        const isUrgent = group === 0;

        if (group !== lastGroup) {
            lastGroup = group;
            const cnt = groupCounts[group] || 0;
            html += `<tr class="earn-group-row"><td colspan="5" style="padding:6px 14px 4px;background:var(--bg-color);border-bottom:1px solid var(--border-color);">
                <span style="font-size:0.67rem;font-weight:700;letter-spacing:0.08em;color:${GROUP_COLORS[group]};">${GROUP_LABELS[group]}</span>
                <span style="font-size:0.67rem;font-weight:500;color:var(--text-muted);margin-left:6px;">• ${cnt}</span>
            </td></tr>`;
        }

        const rowBorder = isUrgent ? 'border-left:3px solid #e74c3c;' : group === 1 ? 'border-left:3px solid #f39c12;' : 'border-left:3px solid transparent;';

        // Consecutive beat streak
        let streak = 0;
        for (const q of quarters.slice(0, 4)) {
            const p = q.surprisePercent != null ? q.surprisePercent : (q.actual != null && q.estimate != null && Math.abs(q.estimate) > 0.001 ? ((q.actual - q.estimate) / Math.abs(q.estimate)) * 100 : null);
            if (p != null && p > 0.5) streak++;
            else break;
        }
        const streakBadge = streak >= 2 ? `<span class="earn-streak">🔥 ${streak}×</span>` : '';

        // EPS trend arrow
        let trendHtml = '';
        if (quarters[0]?.actual != null && quarters[1]?.actual != null) {
            const diff2 = quarters[0].actual - quarters[1].actual;
            if (diff2 > 0.005) trendHtml = '<span class="earn-trend-up">↗</span>';
            else if (diff2 < -0.005) trendHtml = '<span class="earn-trend-down">↘</span>';
            else trendHtml = '<span class="earn-trend-flat">→</span>';
        }

        html += `<tr class="earn-row${isUrgent ? ' earn-row-urgent' : ''}">
            <td style="padding:10px 14px;${rowBorder}">
                <div style="display:flex;align-items:center;gap:9px;">
                    <img src="${logoUrl}" width="26" height="26" style="border-radius:5px;object-fit:contain;background:#fff;padding:1px;flex-shrink:0;border:1px solid var(--border-color);" onerror="this.style.display='none'">
                    <div>
                        <div style="font-weight:700;font-size:0.83rem;letter-spacing:0.02em;">${r.ticker}${streakBadge}</div>
                        <div style="font-size:0.67rem;color:var(--text-muted);max-width:120px;overflow:hidden;text-overflow:ellipsis;">${r.name || ''}</div>
                    </div>
                </div>
            </td>
            <td style="padding:8px 14px;text-align:center;vertical-align:middle;">${miniChart(quarters)}</td>
            <td style="padding:10px 14px;vertical-align:middle;">
                ${current ? `<div class="earn-quarter-label">${quarterLabel(current)}</div>
                <div class="earn-eps-row">
                    <span class="earn-est">Est ${fmtEps(current.estimate)}</span>
                    <span class="earn-arrow">→</span>
                    <span class="earn-act">${fmtEps(current.actual)}</span>
                    ${trendHtml}
            ...
            <td style="padding:10px 14px;vertical-align:middle;white-space:nowrap;">
                ${current ? earnBadge(current.surprisePercent, current.actual, current.estimate) : '—'}
            </td>
            <td style="padding:10px 14px;text-align:center;vertical-align:middle;">${fmtDateCell(r.earnings?.nextDate, group)}</td>
        </tr>`;
    }
